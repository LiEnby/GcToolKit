...
const char* GCAUTHMGR_SM_PATH = "os0:/kd/gcauthmgr_sm.self";
...
int check_version(const char* module) {
	int version = 0x000;
...
SceSblSmCommId sm_start(){
	...
	sm_version = check_version(GCAUTHMGR_SM_PATH);
	int res = ksceSblSmCommStartSmFromFile(0, GCAUTHMGR_SM_PATH, 0, &ctx, &f00d_id);	
	if(res == 0)
		return f00d_id;
	return -1;
}
...
void k_decrypt_secondary_key0(void *data, uint32_t key_id,void *packet9,void *out) {
	...
	PRINT_STR("sm_offset = 0x%08X\n", sm_offset);
	
	int ret = fud_bridge_exec(0x4a000 + sm_offset, 4, decrypt_secondary_key0_args); // decrypt_secondary_key0
	
	PRINT_STR("fud_bridge_exec = 0x%08X\n", ret);
